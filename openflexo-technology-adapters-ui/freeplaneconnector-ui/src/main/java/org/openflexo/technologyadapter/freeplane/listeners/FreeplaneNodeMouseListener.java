...
public class FreeplaneNodeMouseListener extends MNodeMotionListener {
	...
	public void showPopupMenu(final MouseEvent e) {
		...
		if (this.controller.getSelectionManager().getSelection().size() == 1) {
			this.controller.objectWasRightClicked(this.controller.getSelectionManager().getSelection().get(0), new SwingFIBMouseEvent(e));
		}
	}

	/**
	 * See inheritance of freeplane, but just a copy paste with a control statement removed, to force selection.
	 * 
	 * @param e
	 *            MouseEvent
	 */
